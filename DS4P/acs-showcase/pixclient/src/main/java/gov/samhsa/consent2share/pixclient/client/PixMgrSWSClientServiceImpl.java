...
//@Service(value = "PixMgrClientService")
public class PixMgrSWSClientServiceImpl implements PixMgrClientService {
	...
	@Autowired
	RequestXMLToJava requestXMLToJava;

	@Override
	public String addPerson(String reqXMLPath) {

		PIXMgrBean pixMgrBean = new PIXMgrBean();
	...
	@Override
	public String queryPerson(String reqXMLPath) {
	...
	private void getQueryMessage(PRPAIN201310UV02 response,
		...
		for (MCCIMT000300UV01Acknowledgement ackmnt : ackmntList) {
			if (ackmnt.getTypeCode().getCode().equals("AA")) {
				...
				JAXBElement<PRPAMT201307UV02QueryByParameter> qbpjaxb =  cap.getQueryByParameter();
				PRPAMT201307UV02QueryByParameter qbp = qbpjaxb.getValue();
				PRPAMT201307UV02ParameterList plist = qbp.getParameterList();
				List<PRPAMT201307UV02PatientIdentifier> pidList = plist.getPatientIdentifier();
	...
	private void getAddUpdateessage(MCCIIN000002UV01 response, PIXMgrBean pixMgrBean,String msg) {
		// The message has been sent
		// But it doesn't mean we're subscribed successfully
		logger.debug("response ack code:" + response.getAcceptAckCode());
		logger.debug("response type id: " + response.getTypeId());
	...
	private void getSoapExpMessage(SoapFaultClientException sfce, PIXMgrBean pixMgrBean,String msg){
		...
		// Add error
		logger.error("error",
				msg+ " Failure! Validation error! Invalid Message: Request cannot be processed");
		
		if(PixManagerConstants.PIX_ADD.equalsIgnoreCase(msg)) {
			pixMgrBean
			.setAddMessage(msg + " Failure! Validation error! Invalid Message: Request cannot be processed");			
		} else if(PixManagerConstants.PIX_QUERY.equalsIgnoreCase(msg)) {
			pixMgrBean
			.setQueryMessage(msg + " Failure! Validation error! Invalid Message: Request cannot be processed");			
		} else if(PixManagerConstants.PIX_UPDATE.equalsIgnoreCase(msg)) {
			pixMgrBean
			.setUpdateMessage(msg + " Failure! Validation error! Invalid Message: Request cannot be processed");			
		}
		
	}
	
	private void getGeneralExpMessage(Exception e, PIXMgrBean pixMgrBean,String msg){
