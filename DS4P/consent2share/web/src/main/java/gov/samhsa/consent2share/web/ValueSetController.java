...
public class ValueSetController extends AbstractNodeController {
	 ...
	 * @param redirectAttribute the redirect attribute
	 * @param model the model
	 * @return the string
	 */
	...
	public String submitEditValueSetForm(
		...
		attributes
		.addFlashAttribute(MODEL_ATTIRUTE_VALUESETDTO, updated);
	...
	public String batchUpload(HttpServletRequest request, @RequestParam("batch_file") MultipartFile file,
		...
		try {
			...
			valueSetDto = valueSetService.valueSetBatchUpload(valueSetDto, file);
			valueSetDto.setSuccessMessage("Added " + valueSetDto.getRowsUpdated() + " Value Sets");
		...
		redirectAttribute.addFlashAttribute(MODEL_ATTIRUTE_VALUESETDTO, valueSetDto);
	 ...
	 *
	 * @param valueSetService the new value set service
	 */
