			...
			switch($('body').attr("id")){
				...
				case "adminPatientView-page":
					...
					$.ajax({
						  ...
						  success:function(response) {
							  $('table#patient_providers_list > tbody').append("<tr data-npi='" + npi + "' id='provider_row_NPI" + npi + "' data-prov-type='IND' data-prov-id='" + response + "'>" +
							  ...
							  $('table#patient_providers_list').parent().prepend("<div class='modal fade crd-provider-modal' id='crd_provider_modal_NPI" + npi +"' tabindex='-1' role='dialog' aria-labelledby='crd_provider_modal-title_NPI" + npi + "'data-prov-type='IND' data-prov-id='" + response + "' aria-hidden='true'>" +
												  "<div class='modal-dialog'>" +
												    "<div class='modal-content'>" +
												      ...
												      "<div class='modal-body'>" +
												      	"<div>" +
									                		"<div class='provider_record_space' data-prov-type='IND' data-prov-id='" + response + "'>"  +
									                			"<div class='provider_record_box'>" +
																	"<div class='provider_record_header'>" +
																		...
																		"<p class='result_row add_button_space'>" +
																			"<span class='result_field'><button class='btn-delete-prov btn btn-xs btn-danger'><span class='fa fa-minus fa-white'></span></button></span>Remove this Provider." +
																		"</p>" +
																	"</div>" +
						  ...
						  success:function(response) {
							  ...
							  $('div#disclose-list-container').append("<div>" +
							  			"<label for='to" + result4ajaxJSON["providers"][entryId].npi + "'>" +
							  					"<input class='isMadeToList  indv-prov' id='to" + result4ajaxJSON["providers"][entryId].npi + "' " +
							  ...
							  $('div#authorize-list-container').append("<div>" +
									  	"<label for='from" + result4ajaxJSON["providers"][entryId].npi + "'>" +
					  						"<input class='toDiscloseList  indv-prov' id='from" + result4ajaxJSON["providers"][entryId].npi + "' " +
					  							"value='" + result4ajaxJSON["providers"][entryId].npi + "' " +
			...
			case "adminPatientView-page":
				...
				$.ajax({
					  ...
					  data: {npi: result4ajaxJSON["providers"][entryId]["npi"],
						  	 patientId: in_patientId},
					  success:function(response) {
						  $('table#patient_providers_list > tbody').append("<tr data-npi='" + npi + "' id='provider_row_NPI" + npi + "' data-prov-type='ORG' data-prov-id='" + response + "'>" +
						  ...
						  $('table#patient_providers_list').parent().prepend("<div class='modal fade crd-provider-modal' id='crd_provider_modal_NPI" + npi +"' tabindex='-1' role='dialog' aria-labelledby='crd_provider_modal-title_NPI" + npi + "' data-prov-type='ORG' data-prov-id='" + response + "' aria-hidden='true'>" +
								  "<div class='modal-dialog'>" +
								    "<div class='modal-content'>" +
								      "<div class='modal-header'>" +
								        "<button type='button' class='close' data-dismiss='modal' aria-hidden='true'>&times;</button>" +
								      ...
								      "<div class='modal-body'>" +
								      	"<div>" +
					                		"<div class='provider_record_space'data-prov-type='ORG' data-prov-id='" + response + "'>" +
					                			"<div class='provider_record_box'>" +
													"<div class='provider_record_header'>" +
														"<p class='result_row'>" +
			...
			default:
				...
				$.ajax({
					  ...
					  success: function(response) {
						  ...
						  $('div#disclose-list-container').append("<div>" +
						  			"<label for='to" + result4ajaxJSON["providers"][entryId].npi + "'>" +
						  					"<input class='isMadeToList org-prov' id='to" + result4ajaxJSON["providers"][entryId].npi + "' " +
						  						"value='" + result4ajaxJSON["providers"][entryId].npi + "' " +
						  ...
						  $('div#authorize-list-container').append("<div>" +
						  			"<label for='from" + result4ajaxJSON["providers"][entryId].npi + "'>" +
						  					"<input class='toDiscloseList org-prov' id='from" + result4ajaxJSON["providers"][entryId].npi + "' " +
						  						"value='" + result4ajaxJSON["providers"][entryId].npi + "' " +
...
function lookup(){
		...
		$("#providerSearchSelect-modal .search-loading").show();
		...
		var myAjaxCall= $.ajax({
        	dataType: "json",
			url: "../Administrator/providerSearch.html",
			type:"GET",
			async:true, 
		...
		function killAjaxCall(){  // if no response, abort both getJson requests
		    ...
		    myAjaxCall.abort();
		    setTimeout( function() { $("#providerSearchSelect-modal .search-loading").fadeOut({ duration: 400}); }, 200 );
		    $("#noResponse").show();
		    }
