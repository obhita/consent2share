...
public class ConsentControllerTest {
	...
	@Mock
	MedicalSectionService medicalSectionServiceImpl;
	...
	public void testConsentAddGet_Check_if_all_attributes_are_added_and_right_view_is_returned()
		...
		mockMvc.perform(get("/consents/addConsent.html"))
				.andExpect(model().attribute("npiList", 
						matching(
								(Set set) -> set.size() == 2 && set.contains(orgNpi) && set.contains(indNpi)
								)))
				.andExpect(
						model().attribute(
								"patient_lname",
