...
public class CodeSystemVersionServiceImplTest {
	...
	@InjectMocks
	CodeSystemVersionService vst=new CodeSystemVersionServiceImpl();
	...
	public void testCreateCodeSystemVersion_throw_CodeSystemNotFoundException() throws CodeSystemNotFoundException{
	...
	@Test
	public void testDeleteCodeSystemVersion() throws CodeSystemVersionNotFoundException {
		
		CodeSystemVersion deleted = mock(CodeSystemVersion.class);
		when(codeSystemVersionRepository.findOne(anyLong())).thenReturn(deleted);
		
		CodeSystemVersionDto codeSystemVersionDto = mock(CodeSystemVersionDto.class);
		when(valueSetMgmtHelper.createCodeSystemVersionDtoFromEntity(deleted)).thenReturn(codeSystemVersionDto);
		
		Assert.assertEquals(vst.delete((long)1), codeSystemVersionDto);
	}
	
	@Test(expected = CodeSystemVersionNotFoundException.class)
	public void testDeleteCodeSystemVersion_throw_CodeSystemVersionNotFoundException() throws CodeSystemVersionNotFoundException {
	...
	public void testfindAll(){
		...
		List<CodeSystemVersion> codeSystemVersionsMock=(List<CodeSystemVersion>)mock(List.class);
		when(codeSystemVersionRepository.findAll()).thenReturn(codeSystemVersionsMock);
		
		List<CodeSystemVersionDto> codeSystemVersionDtos=new ArrayList();
		CodeSystemVersionDto codeSystemVersionDto=mock(CodeSystemVersionDto.class);
		codeSystemVersionDtos.add(codeSystemVersionDto);
		when(valueSetMgmtHelper.convertCodeSystemVersionEntitiesToDtos(codeSystemVersionsMock)).thenReturn(codeSystemVersionDtos);
	...
	public void testfindId() throws CodeSystemNotFoundException{
		...
		when(codeSystemVersionRepository.findOne(anyLong())).thenReturn(codeSystemVersionMock);
		when(codeSystemRepository.findAll()).thenReturn(codeSystemsMock);		
		
		when(valueSetMgmtHelper.createCodeSystemVersionDtoFromEntity(codeSystemVersionMock)).thenReturn(codeSystemVersionDtoMock);
		
		assertEquals(vst.findById((long)1),codeSystemVersionDtoMock);
	}
	
	@Test
	public void testUpdateCodeSystemVersion() throws CodeSystemVersionNotFoundException, CodeSystemNotFoundException{
		...
		when(codeSystemRepository.findOne(anyLong())).thenReturn(codeSystemMock);
	...
	@Test(expected = CodeSystemVersionNotFoundException.class)
	public void testUpdateCodeSystemVersion_throw_CodeSystemVersionNotFoundException() throws CodeSystemVersionNotFoundException, CodeSystemNotFoundException {
	...
	public void testCodeSystemVersionCSDto() throws CodeSystemNotFoundException {
		...
		when(valueSetMgmtHelper
				.convertCodeSystemEntitiesToMap(codeSystemsMock)).thenReturn(codeSystemCategoriesMap);
