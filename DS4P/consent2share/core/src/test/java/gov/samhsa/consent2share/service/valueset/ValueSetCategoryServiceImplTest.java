...
public class ValueSetCategoryServiceImplTest {
	...
	@InjectMocks
	ValueSetCategoryService vst=new ValueSetCategoryServiceImpl();
	...
	public void testCreateValueSetValueSetCategory(){
		...
		when(valueSetCategoryRepository.save(valueSetCategory)).thenReturn(valueSetCategory);
		
		when(valueSetMgmtHelper.createValuesetCategoryDtoFromEntity(any(ValueSetCategory.class))).thenReturn(created);
	...
	@Test
	public void testDeleteValueSetCategory() throws ValueSetCategoryNotFoundException {
		...
		when(valueSetMgmtHelper.createValuesetCategoryDtoFromEntity(deleted)).thenReturn(valueSetCategoryDto);
		
		Assert.assertEquals(vst.delete((long)1), valueSetCategoryDto);
	}
	
	@Test(expected = ValueSetCategoryNotFoundException.class)
	public void testDeleteVelueSetCategory_throw_ValueSetCategoryNotFoundException() throws ValueSetCategoryNotFoundException {
	...
	public void testfindAll(){
		...
		List<ValueSetCategory> valueSets=(List<ValueSetCategory>)mock(List.class);
		when(valueSetCategoryRepository.findAll()).thenReturn(valueSets);
		
		List<ValueSetCategoryDto> valueSetDtos=new ArrayList();
		ValueSetCategoryDto valueSetDto=mock(ValueSetCategoryDto.class);
		valueSetDtos.add(valueSetDto);
		when(valueSetMgmtHelper.convertValueSetCategoryEntitiesToDtos(valueSets)).thenReturn(valueSetDtos);
	...
	public void testfindId(){
		...
		when(valueSetCategoryRepository.findOne(anyLong())).thenReturn(valueSetCategory);
		
		ValueSetCategoryDto valueSetCategoryDto=mock(ValueSetCategoryDto.class);
		when(valueSetMgmtHelper.createValuesetCategoryDtoFromEntity(valueSetCategory)).thenReturn(valueSetCategoryDto);
		
		assertEquals(vst.findById((long)1),valueSetCategoryDto);
	}
	
	@Test
	public void testUpdateValueSetCategory() throws ValueSetCategoryNotFoundException{
	...
	@Test(expected = ValueSetCategoryNotFoundException.class)
	public void testUpdateValueSetCategory_throw_ValueSetCategoryNotFoundException() throws ValueSetCategoryNotFoundException {
		ValueSetCategoryDto updated=mock(ValueSetCategoryDto.class);
		when(valueSetCategoryRepository.findOne(anyLong())).thenReturn(null);
		vst.update(updated);
	...
	public void testFindAllValueSetCategoriesAddConsentFieldsDto(){
		...
		// Act
		List<AddConsentFieldsDto> sensitivityPolicyDto = vst.findAllValueSetCategoriesAddConsentFieldsDto();
		...
		assertEquals(vscMockDescription, sensitivityPolicyDto.get(0).getDescription());
		...
		assertEquals(vscMock2Description, sensitivityPolicyDto.get(1).getDescription());
	}	
}
