...
public class PatientLegalRepresentativeAssociationServiceTest {

	@InjectMocks
	PatientLegalRepresentativeAssociationService patientLegalRepresentativeAssociationService = new PatientLegalRepresentativeAssociationServiceImpl();
	...
	public void testDeletePatientLegalRepresentativeById() {
	 ...
	 when(association.getPatientLegalRepresentativeAssociationPk()).thenReturn(associationPk);
	 ...
	 when(patientLegalRepresentativeAssociationRepository.findByPatientLegalRepresentativeAssociationPkLegalRepresentativeId(anyLong())).thenReturn(associations);
	 when(patientRepository.findByUsername(anyString())).thenReturn(patient2);
	 when(patient2.getId()).thenReturn((long)1);

	 patientLegalRepresentativeAssociationService.deletePatientLegalRepresentativeById(anyLong());
	 
	 verify(patientLegalRepresentativeAssociationRepository).delete(association);
	 }
	...
	public void testFindAllPatientLegalRepresentativeAssociationDtos() {
		...
		when(patientLegalRepresentativeAssociationRepository.findAll()).thenReturn(allAssociations);
		when(association.getPatientLegalRepresentativeAssociationPk()).thenReturn(associationPk);
		when(associationPk.getPatient()).thenReturn(patient2);
		when(associationPk.getLegalRepresentative()).thenReturn(legalRep);
		when(patient2.getId()).thenReturn((long)1);
		when(modelMapper.map(association, PatientLegalRepresentativeAssociationDto.class)).thenReturn(associationDto);
