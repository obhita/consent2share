	...
	public String parse(SamlAssertionWrapper assertionWrapper, String urn) {
		try {
			...
			if (asList == null || asList.size() == 0) {
				...
				for (Iterator<AttributeStatement> it = asList.iterator(); it
						.hasNext();) {
					...
					if (attList == null || attList.size() == 0) {
						...
						for (Iterator<Attribute> it2 = attList.iterator(); it2
								.hasNext();) {
							...
							if (!att.getName().equals(urn)) {
								...
								if (xoList == null || xoList.size() < 1
									...
									additionalValue = xmlObj.getDOM()
											.getFirstChild().getTextContent();
									return xmlObj.getDOM().getFirstChild()
											.getTextContent();
