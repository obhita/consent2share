...
public class BufferedHttpStreamReader extends InputStreamReader
{
  ...
  public BufferedHttpStreamReader(InputStream in) throws UnsupportedEncodingException
  {
    this(in, "UTF-8", DEFAULT_BUFFER_SIZE);
  }

  public BufferedHttpStreamReader(InputStream in, String charsetName, int bufferSize) throws UnsupportedEncodingException
  {
    ...
    // for the Host header, use the specified domain name if given
    String echoSignDomainName = System.getenv(ECHOSIGN_DOMAIN_NAME_ENV_NAME);
    hostHeaderEchoSign = HOST_HEADER + ((echoSignDomainName != null) ? echoSignDomainName : ECHOSIGN_DOMAIN_NAME);
